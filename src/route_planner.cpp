...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    current_node->neighbors[1];
    for (std::size_t  i=0; i<current_node->neighbors.size(); i++) {
        current_node->neighbors[i]->parent = current_node;
        current_node->neighbors[i]->g_value = current_node->neighbors[i]->distance(*end_node);
...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(open_list.begin(), open_list.end(), [](const auto &front, const auto &back){
        return front->h_value + front->g_value < back->h_value + back->g_value;
    });
